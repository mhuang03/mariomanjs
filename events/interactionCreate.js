...
module.exports = (client, inter) => {
    ...
    if (DJ.enabled && DJ.commands.includes(command) && !inter.member._roles.includes(inter.guild.roles.cache.find(x => x.name === DJ.roleName).id)) return inter.reply({ embeds: [ new EmbedBuilder().setColor('#ff0000').setDescription(`❌ | This command is reserved For members with \`${DJ.roleName}\` role`)], ephemeral: true, })
    if (Active.enabled && Active.commands.includes(command) && !inter.member._roles.includes(inter.guild.roles.cache.find(x => x.name === Active.roleName).id)) return inter.reply({ embeds: [ new EmbedBuilder().setColor('#ff0000').setDescription(`❌ | This command is reserved For members with \`${Active.roleName}\` role`)], ephemeral: true, })
