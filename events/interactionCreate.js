...
module.exports = (client, inter) => {
    if (inter.type === InteractionType.ApplicationCommand) {
        ...
        const Active = client.config.opt.Active;
    ...
    if (DJ.enabled && DJ.commands.includes(command.name) && !inter.member._roles.includes(inter.guild.roles.cache.find(x => x.name === DJ.roleName).id)) return inter.reply({ embeds: [ new EmbedBuilder().setColor('#ff0000').setDescription(`❌ | This command is reserved For members with \`${DJ.roleName}\` role`)], ephemeral: true, })
    if (Active.enabled && Active.commands.includes(command.name) && !inter.member._roles.includes(inter.guild.roles.cache.find(x => x.name === Active.roleName).id)) return inter.reply({ embeds: [ new EmbedBuilder().setColor('#ff0000').setDescription(`❌ | This command is reserved For members with \`${Active.roleName}\` role`)], ephemeral: true, })
